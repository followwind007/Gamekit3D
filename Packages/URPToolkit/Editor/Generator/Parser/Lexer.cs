...
namespace GameApp.URPToolkit.Parser
{
    public class Lexer : IEnumerable<Token>, IEnumerator<Token>
    {
        ...
        private Token GetToken()
        {
            ...
            if (_settings.Symbols != null)
            {
                ...
                for (var i = symbol.Length; i > 0; i--, symbol = symbol.Substring(0, i))
                {
                    if (_settings.Symbols.TryGetValue(symbol, out var symbolId))
                    {
                        Skip(i);
                        var symbolText = (_behavior & LexerBehavior.PersistTokenText) != 0 ? symbol : null;
                        return new Token(TokenType.Symbol, symbol, symbolText, symbolId, _start, _position, _lineBegin, _lineNumber, _lineBegin, _lineNumber);
                    }
                }
            }
        ...
        private Token GetKeywordOrIdentifierToken(char currentChar)
        {
            ...
            return new Token(tokenType, val, GetTokenText(), id, _start, _position, _lineBegin, _lineNumber, _lineBegin, _lineNumber);
        }
        ...
        private Token GetNumberToken(char currentChar)
        {
            ...
            else
            {
                ...
                else if (decimal.TryParse(val, out var decimalVal))
                {
        ...
        bool IEnumerator.MoveNext() => GetNextToken().type != TokenType.End;
    ...
    public sealed class Token
    {
        ...
        public Token(TokenType type, object value, string text, int id, int startPosition, int endPosition, int lineBegin, int lineNumber, int endLineBegin, int endLineNumber)
        {
            ...
            this.endLineBegin = endLineBegin;
    ...
    public sealed class LexerSettings : ICloneable
    {
        ...
        public LexerSettings Clone()
        {
            ...
            {
                var ignoreCase = (settings.Options & LexerOptions.IdentIgnoreCase) != 0;
